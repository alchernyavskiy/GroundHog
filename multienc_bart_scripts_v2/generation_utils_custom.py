...
class GenerationMixin:
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        ...
        encoder_outputs_full = []
        attention_mask_full = copy.deepcopy(encoder_kwargs['attention_mask'])
        for i in range(len(self.model.encoders)):
            start_idx = sum(self.model.encoder_lens[:i]) if i != 0 else 0
            end_edx = start_idx + self.model.encoder_lens[i]
            encoder_kwargs[model_input_name] = inputs_tensor[:,start_idx:end_edx,...]
            encoder_kwargs['attention_mask'] = attention_mask_full[:,start_idx:end_edx,...]
            encoder_outputs_full.append(
                self.model.encoders[i](
                    **encoder_kwargs,
                )[0]
            )
        encoder_outputs = BaseModelOutput(
                last_hidden_state=encoder_outputs_full,
                hidden_states=None,
                attentions=None,
        )
    ...
    def beam_sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            probs = nn.functional.softmax(next_token_scores, dim=-1)
            prob_positive = int((probs > 0).sum(dim=1).min())
            
            next_tokens = torch.multinomial(probs, num_samples=min(2 * num_beams, prob_positive))
